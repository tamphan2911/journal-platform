...
const sections = [
  {
    ...
    items: [
      ...
      { href: "/workspace/admin/media", label: "Thư viện ảnh", icon: Images },
      { href: "/workspace/admin/settings", label: "Header & chân trang", icon: Settings2 },
      { href: "/workspace/admin/users", label: "Người dùng", icon: Users },
    ],
  },
  {
    label: "Quản lý chuyên san",
    items: [
      { href: "/workspace/admin/submissions", label: "Bản thảo & phản biện", icon: FileText },
...
export function AdminNav() {
  ...
  return (
    <nav aria-label="Quản trị hệ thống" className="grid gap-4 lg:gap-6">
      {sections.map((section) => (
        <section key={section.label} aria-label={section.label}>
          <p className="mb-2 px-3 text-[10px] font-extrabold uppercase text-[var(--uel-brand-blue)]/65">
            {section.label}
          </p>
